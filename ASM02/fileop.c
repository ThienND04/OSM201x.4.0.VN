   ...
   if (numOfArg >= 3){
      if(strcmp(args[1],"ls") == 0){
         ...
         listDir(args[2]);
		   ...
		   //TO DO
         int errcode = makeDir(args[2]);
         if(errcode != 0) printf("%s\n", strerror(errcode));
         
	   } else if(strcmp(args[1],"cp") == 0){
         if(numOfArg >= 3){
           // TO DO
           int errcode = copyFile(args[2], args[3]);
           if(errcode != 0) printf("%s\n", strerror(errcode));
         } else
		   ...
		   //TO DO
         if(numOfArg >= 3) {
            int errcode = mvFile(args[2], args[3]);
           if(errcode != 0) printf("%s\n", strerror(errcode));
         }
         else{
            printf ("%s: missing destination file operand after %s\n", args[1], args[2]);
...
int mvFile (char *source, char *dest){
   ...
   // 1. Process rename
   if(copyFile(source, dest) != 0) return errno;
